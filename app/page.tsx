...
const StarIcon = ({ filled }: { filled: boolean }) => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill={filled ? "#f98f44" : "#e4e4e7"} stroke="none"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
);
    ...
    <div style={{ fontFamily: "inherit" }}>
      ...
      <nav style={{ position: "fixed", top: 16, left: "50%", transform: "translateX(-50%)", width: "92%", maxWidth: 1200, background: "#fff", border: "1px solid #f1f5f9", borderRadius: 24, padding: "0 20px", height: 68, display: "flex", alignItems: "center", justifyContent: "space-between", zIndex: 100, boxShadow: "0 2px 20px rgba(0,0,0,0.06)" }}>
        ...
        <div className="desktop-nav" style={{ gap: 10, alignItems: "center" }}>
          ...
          <Link href="/contact" style={{ background: "#000", borderRadius: 100, padding: "10px 18px", fontSize: 13, fontWeight: 700, color: "#fff", textDecoration: "none" }}>Contact Us</Link>
        </div>
        <MobileNav />
      </nav>
      ...
      <section className="hero-section">
        <div ref={heroRef} className="hero-box" style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div style={{ position: "absolute", bottom: 32, zIndex: 1 }}>
      ...
      <section className="section-sm">
        <div className="container">
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
            ...
            <Link href="/inventory" style={{ background: "#000", color: "#fff", borderRadius: 100, padding: "12px 24px", fontWeight: 700, fontSize: 14, textDecoration: "none", whiteSpace: "nowrap" }}>View All</Link>
          </div>
          <div className="grid-auto">
            {cars.map((car) => (
              <Link key={car.id} href={`/cars/${car.slug}`} className="car-card" style={{ opacity: car.sold ? 0.6 : 1 }}>
                <div className="car-card-img">
                  <img src={car.thumbnail} alt={car.imageAlt} loading="lazy" />
                  ...
                  {car.sold && <span style={{ position: "absolute", top: 12, left: 12, background: "#dc2626", color: "#fff", borderRadius: 100, padding: "4px 12px", fontSize: 12, fontWeight: 600 }}>Sold</span>}
                </div>
                <div className="car-card-body">
                  ...
                  <div style={{ marginBottom: 14 }}>
                    <span style={{ fontSize: 21, fontWeight: 900, color: "#000" }}>${car.price.toLocaleString()}</span>
                  </div>
        ...
        <div className="container">
          ...
          <div className="why-grid">
            {[
              ...
              { icon: <TagIcon />, title: "Best Prices", desc: "Competitive prices in dollars with flexible payment options." },
      ...
      <section className="section">
        <div className="container">
          ...
          <p className="section-sub" style={{ textAlign: "center" }}>Real experiences from real customers</p>
      ...
      <section className="section">
        <div className="container">
          <div style={{ background: "#0a0a0a", borderRadius: 32, padding: "60px 32px", textAlign: "center" }}>
            <h2 style={{ fontSize: 30, fontWeight: 900, color: "#fff", marginBottom: 12 }}>Ready to Find Your Next Vehicle?</h2>
            <p style={{ color: "#aaa", marginBottom: 32, maxWidth: 500, margin: "0 auto 32px", fontSize: 14 }}>Chat with us on WhatsApp or visit our showroom on Zambezi Road, Lusaka.</p>
      ...
      <footer style={{ background: "#000", borderRadius: "24px 24px 0 0", padding: "48px 32px 32px", margin: "0 12px" }}>
        <div className="container" style={{ padding: 0 }}>
          <div className="footer-inner">
            <div>
              <p style={{ color: "#fafafa", fontWeight: 900, fontSize: 22, marginBottom: 16 }}>Xhanti AutoSales</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <p style={{ color: "#aaa", fontSize: 13, display: "flex", alignItems: "center", gap: 8 }}><LocationIcon /> 54 Zambezi Road, Lusaka, Zambia</p>
                <p style={{ color: "#aaa", fontSize: 13, display: "flex", alignItems: "center", gap: 8 }}><PhoneIcon /> {dealerInfo.phone}</p>
                <p style={{ color: "#25D366", fontSize: 13, display: "flex", alignItems: "center", gap: 8 }}><WhatsAppIcon /> {dealerInfo.whatsapp}</p>
                <p style={{ color: "#aaa", fontSize: 13, display: "flex", alignItems: "center", gap: 8 }}><ClockIcon /> {dealerInfo.hours}</p>
              </div>
            </div>
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 20, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
            <p style={{ color: "#aaa", fontSize: 12 }}>© 2026 Xhanti AutoSales. All Rights Reserved.</p>
            <p style={{ color: "#aaa", fontSize: 12 }}>Lusaka, Zambia</p>
          </div>
        </div>
      </footer>
    </div>
