...
const links = [
  ...
  { href: "/inventory?condition=New", label: "New", match: "/inventory" },
  { href: "/inventory?condition=Used", label: "Used", match: "/inventory" },
    ...
    <nav style={{ position: "fixed", top: 16, left: "50%", transform: "translateX(-50%)", width: "92%", maxWidth: 1200, background: "#fff", border: "1px solid #f1f5f9", borderRadius: 24, padding: "0 20px", height: 68, display: "flex", alignItems: "center", justifyContent: "space-between", zIndex: 100, boxShadow: "0 2px 20px rgba(0,0,0,0.06)" }}>
      ...
      <div className="desktop-nav" style={{ gap: 28 }}>
        {links.map(({ href, label, match }) => (
          <Link key={href} href={href} style={{ fontWeight: 700, fontSize: 14, textDecoration: "none", color: isActive(match || href) ? "#1453ae" : "#000", borderBottom: isActive(match || href) ? "2px solid #1453ae" : "2px solid transparent", paddingBottom: 2 }}>{label}</Link>
        ))}
      </div>
